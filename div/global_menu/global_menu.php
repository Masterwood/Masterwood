<nav><span id="global"><span class="menu-waku">MENU</span></span>
	<ul id="nav">
		<span class="w_5"></span>
		 <li class="ml_3"><a href="index">HYPER AMBITION</a>
     	</li>
        <li><a href="business/index.html"><img src="http://masterwood.sakura.ne.jp/img/icon/show.png" class="nav_icon">サービス紹介</a>
            <span id="s2"></span>
           	<ul class="subs">
                <li>コーチング
                	<ul>
                		<li><a href="">WEB制作</a></li>
                    	<li><a href="">WEB運用</a></li>
                	</ul>
                </li>
                <li>サイト制作
                	<ul>
                		<li><a href="">LP制作</a></li>
                		<li><a href="">WEB制作</a></li>
                    	<li><a href="">オウンドメディア</a></li>
                	</ul>
                </li>
                <li>コンサルティング
                	<ul>
                		<li><a href="">SEO施策</a></li>
                    	<li><a href="">集客</a></li>
                	</ul>
                </li>
            </ul>
        </li>
        <li><a href=""><img src="http://masterwood.sakura.ne.jp/img/icon/zisseki.png" class="nav_icon">運用メディア</a>
            <span id="s3"></span>
            <ul class="subs">
                <li>ヘルスケア・医療メディア
	                <ul>
	                    <li><a href="http://hospitalcloud.net/">Hospital Cloud</a></li>
	                </ul>
                </li>
                <li>学習メディア
	                <ul>
	                    <li><a href="">近くでEnglish</a></li>
	                </ul>
                </li>
                <li>お蔵入りのコメディーサイト
	                <ul>
	                     <li><a href="http://masterwood.sakura.ne.jp/masterwood/">MasterWood</a></li>
	                </ul>
                </li>
            </ul>
     	</li>
     	<li><a href="office/index.html">事業情報</a>
     		<span id="s4"></span>
     	</li>
     	<li><a href="">お問い合わせ</a>
     		<span id="s4"></span>
     		<ul class="subs">
     			<li><a href="">お問い合わせ</a>
     				<ul>
     					<li><a href=""></a></li>
     				</ul>
     			</li>
     		</ul>
     	</li  class="mr_3">
    </ul>
</nav>
